...
def call_scraper_tool_node(state: AgentState) -> dict:
    """Prompts the LLM to use the scraping tool with the candidate's URL."""
...
def handle_tool_error_node(state: AgentState) -> dict:
    """Provides a graceful exit if the scraping tool fails."""
...
def log_answer_node(state: AgentState) -> dict:
    """Silently records the question-and-answer pair for the final scorecard."""
...
def give_acknowledgement_node(state: AgentState) -> dict:
    """Provides a simple, human-like acknowledgement after a normal answer."""
...
def handle_give_up_node(state: AgentState) -> dict:
    """Provides an encouraging response if the candidate doesn't know an answer."""
...
def handle_unprofessional_node(state: AgentState) -> dict:
    """Provides a firm but polite correction for unprofessional language."""
